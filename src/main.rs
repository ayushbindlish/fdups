...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let delete = args.contains(&String::from("--delete")); // Check if --delete flag is set
    let path = args.get(1).expect("Missing directory argument"); // Get directory path from arguments
